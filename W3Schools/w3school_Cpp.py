...
mainURL="https://www.w3schools.com/cpp/default.asp"
...
for slug in pages:
    urls.append('https://www.w3schools.com/cpp/' + slug.get('href'))
...
for url in urls:
    ...
    if link is not None:
        for lin in link:
            if lin.get('href') is not None and not lin.get('href').startswith('http'):
                pageLinks.append('https://www.w3schools.com/cpp/' + lin.get('href'))
            else:
    ...
    if images is not None:
        for img in images:
            pageImageURLs.append('https://www.w3schools.com/cpp/' + img.get('src'))
