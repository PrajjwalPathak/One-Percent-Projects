# Web Scraping CSS Tutorial Data from W3Schools

import requests
from bs4 import BeautifulSoup
import pandas as pd

mainURL = "https://www.w3schools.com/css/default.asp"
...
for slug in pages:
    if slug.get('href') == 'css_examples.asp':
        continue
    urls.append('https://www.w3schools.com/css/' + slug.get('href'))
...
for url in urls:
    ...
    if link is not None:
        for lin in link:
            if lin.get('href') is not None and not lin.get('href').startswith('http'):
                pageLinks.append('https://www.w3schools.com/css/' + lin.get('href'))
            else:
    ...
    if images is not None:
        for img in images:
            pageImageURLs.append('https://www.w3schools.com/css/' + str(img.get('src')))
...
df.to_csv('W3Schools-CSS-Data.csv')
