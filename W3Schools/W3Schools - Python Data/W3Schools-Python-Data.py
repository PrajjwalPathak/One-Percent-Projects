# Web Scraping Python Tutorial Data from W3Schools

import requests
from bs4 import BeautifulSoup
import pandas as pd

mainURL = "https://www.w3schools.com/python/default.asp"
...
for slug in pages:
    urls.append('https://www.w3schools.com/python/' + slug.get('href'))
...
for url in urls:
    ...
    if link is not None:
        for lin in link:
            if lin.get('href') is not None and not lin.get('href').startswith('http'):
                pageLinks.append('https://www.w3schools.com/python/' + lin.get('href'))
            else:
    ...
    if images is not None:
        for img in images:
            pageImageURLs.append('https://www.w3schools.com/python/' + img.get('src'))
...
df.to_csv('W3Schools-Python-Data.csv')
